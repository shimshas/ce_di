...
class KaseyaVSAPlugin(IotPluginBase):
    ...
    def validate(self, configuration: dict) -> ValidationResult:
        ...
        if not isinstance(base_url, str) or not self._validate_url(base_url):
            ...
            return ValidationResult(success=False, message=err_msg)
        if "username" not in configuration or not username:
        ...
        if not isinstance(username, str):
            ...
            self.logger.error(
                ...
                "Invalid Username in the configuration parameters.".format(
                    self.log_prefix
                ),
                error_code="IoT_PLUGIN_150024",
            )
            return ValidationResult(success=False, message=err_msg)
    ...
    def pull(self):
        ...
        username = self.configuration.get("username", "").strip()

...
class KaseyaVSAPlugin(IotPluginBase):
    ...
    def get_assets(self, record):
        ...
        try:
            ...
            try:
                if isinstance(_ip_raw, str) and _ip_raw.startswith("["):
                    import ast as _ast
                    _ip_raw = _ast.literal_eval(_ip_raw)
            except Exception:
                _ip_raw = None
            if isinstance(_ip_raw, list):
                _all_ips = []
                for _iface in _ip_raw:
                    _ip_val = str(_iface.get("IpV4", ""))
                    if _ip_val and self._is_valid_ipv4(_ip_val):
                        _all_ips.append(_ip_val)
                # Prefer private (RFC 1918) IPs over public
                for _candidate in _all_ips:

...
class KaseyaVSAException(Exception):
    """KaseyaVSAException exception class."""
    pass


class KaseyaVSAPlugin(IotPluginBase):
    """Kaseya VSA plugin class."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize Kaseya VSA plugin class."""
        super().__init__(
            name,
            *args,
            **kwargs,
    ...
    def handle_error(self, response, logger_msg):
        ...
        raise KaseyaVSAException(err_msg)
    ...
    def _api_helper(self, request, logger_msg, is_handle_error_required=True):
        ...
        try:
            ...
            raise KaseyaVSAException(toast_msg)
            ...
            raise KaseyaVSAException(toast_msg)
        except KaseyaVSAException as err:
            raise err
        except Exception as exp:
            err_msg = (
    ...
    def is_valid_timestamp(
        ...
        try:
            if not timestamp:
                invalid_fields.append(field_name)
                return None
            dt = datetime.strptime(timestamp, format).replace(microsecond=0)
            return (
    ...
    def validate(self, configuration: dict) -> ValidationResult:
        ...
        credentials = configuration.get("username", "").strip()
        if ":" in credentials:
        ...
        if "base_url" not in configuration or not base_url:
            err_msg = "Kaseya VSA URL is a required field."
            self.logger.error(
        ...
        if not isinstance(base_url, str) or not self._validate_url(base_url):
            err_msg = "Invalid Kaseya VSA URL provided."
            self.logger.error(
                message=f"{self.log_prefix}: Validation error occurred. "
                        f"Error: {err_msg}",
                error_code="IoT_PLUGIN_150023",
            )
            return ValidationResult(success=False, message=err_msg)
        if "username" not in configuration or not credentials:
            err_msg = "API Credentials is a required field."
            self.logger.error(
                message=f"{self.log_prefix}: Validation error occurred. "
                        f"Error: {err_msg}",
    ...
    def get_assets(self, record):
        ...
        try:
            ...
            # MAC address
            mac_address = record.get("IpAddresses", "")
            ...
            # IP address
            ip_address = record.get("LocalIpAddresses")
            if not self._is_valid_ipv4(str(ip_address)):
    ...
    def pull(self):
        ...
        params = {
            "$top": LIMIT,
            "$skip": 0,
            "sysparm_fields": "AssetInfo,ClientVersion,Description,Identifier,IpAddresses,LastSeenOnline,LocalIpAddresses,Name,SiteId,Tags,Type",
        }
        ...
        while True:
            try:
                response = self._api_helper(
                    ...
                    "fetching assets from kaseya vsa platform",
                    True,
                )
